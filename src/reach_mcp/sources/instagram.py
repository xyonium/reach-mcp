"""Instagram via ScrapeCreators (free-tier key). Off by default."""
...
class Instagram(Source):
    name = "instagram"
    description = "Instagram search via ScrapeCreators (free-tier key)."
    ...
    async def fetch(self, query: str, days: int, limit: int) -> list[Row]:
        if not self.available():
            return []
        client = get_client()
        return await scrape_search(client, "instagram", query, limit)
