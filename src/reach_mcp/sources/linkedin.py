"""LinkedIn via ScrapeCreators (free-tier key). Off by default."""
...
class LinkedIn(Source):
    name = "linkedin"
    description = "LinkedIn search via ScrapeCreators (free-tier key)."
    ...
    async def fetch(self, query: str, days: int, limit: int) -> list[Row]:
        if not self.available():
            return []
        client = get_client()
        return await scrape_search(client, "linkedin", query, limit)
