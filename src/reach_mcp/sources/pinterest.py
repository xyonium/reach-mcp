"""Pinterest via Apify (primary, free $5/mo) + OpenCLI (optional desktop free)
...
from reach_mcp.sources._apify import fetch_pinterest as _apify_fetch
...
class Pinterest(Source):
    name = "pinterest"
    description = (
        "Pinterest via Apify (free $5/mo; APIFY_API_TOKEN) or OpenCLI (desktop, "
    ...
    async def fetch(self, query: str, days: int, limit: int) -> list[Row]:
        ...
        if _has_opencli():
            rows = await cli_search("pinterest", query, limit)
            if rows:
                return rows
        if os.environ.get("SCRAPECREATORS_API_KEY", "").strip():
            return await scrape_search(get_client(), "pinterest", query, limit)
        return []
