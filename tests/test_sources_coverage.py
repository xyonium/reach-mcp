...
async def test_bluesky_parses_posts():
    c = AsyncMock()
    c.get_json = AsyncMock(return_value={"posts": [{
...
async def test_github_parses_repos():
    ...
    c.get_json = AsyncMock(return_value={"items": [{
        "id": 1, "full_name": "foo/bar", "name": "bar",
        "html_url": "https://github.com/foo/bar",
        "owner": {"login": "foo"}, "pushed_at": "2026-07-01T00:00:00Z",
        "stargazers_count": 42, "forks_count": 7, "description": "a repo",
    }]})
    set_client(c)
...
@pytest.mark.asyncio
async def test_xiaoyuzhou_parses_episodes():
    c = AsyncMock()
    c.get_json = AsyncMock(return_value={"data": {"episode_list": [{
        "eid": "e1", "title": "Podcast ep", "url": "https://xyz.fm/e1",
        "podcast": {"title": "Cast"}, "pub_date": "2026-07-01",
    }]}})
    set_client(c)
...
async def test_instagram_uses_apify_when_token_set(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        AsyncMock(return_value=[Row(source="instagram", id="1", title="T",
                                    url="https://instagram.com/p/1",
                                    author="u", date=None, engagement={}, text="x")]),
    )
...
async def test_pinterest_uses_apify_when_token_set(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        AsyncMock(return_value=[Row(source="pinterest", id="1", title="Pin",
...
async def test_apify_to_row_normalizes_fields():
    ...
    row = _to_row({
        "id": "p1", "text": "caption text", "url": "https://x.com/p1",
        "authorMeta": {"username": "user1"},
        "timestamp": "2026-07-01", "likesCount": 5, "commentsCount": 2,
        "playCount": 100,
    }, "tiktok")
